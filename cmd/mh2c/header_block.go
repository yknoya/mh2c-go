...
func consumeHeaderBlockForDisplay(
	...
	case frame.HeadersFrame:
		if typed.Header().Flags&frame.FlagHeadersEndHeaders != 0 {
			...
			return report.Fields, report.Warnings, typed.Header().StreamID, typed.Header().Flags&frame.FlagHeadersEndStream != 0, err
		}
		*pendingStream = typed.Header().StreamID
		*pendingBlock = append([]byte(nil), typed.BlockFragment...)
		*pendingEnd = typed.Header().Flags&frame.FlagHeadersEndStream != 0
	case frame.ContinuationFrame:
		if *pendingStream == 0 {
			return nil, nil, 0, false, fmt.Errorf("unexpected CONTINUATION frame on stream %d", typed.Header().StreamID)
		}
		if typed.Header().StreamID != *pendingStream {
			return nil, nil, 0, false, fmt.Errorf("CONTINUATION stream mismatch: got %d, want %d", typed.Header().StreamID, *pendingStream)
		}
